...
# web/appcast.xml (it auto-deploys to https://cycler.caiano.com/appcast.xml).
#
...
URL="https://github.com/hcaiano/cycler/releases/download/v${VERSION}/$(basename "${DMG}")"
NOTES="https://github.com/hcaiano/cycler/releases/tag/v${VERSION}"
DATE="$(date -u +'%a, %d %b %Y %H:%M:%S +0000')"

cat > web/appcast.xml <<XML
<?xml version="1.0" encoding="utf-8"?>
<!-- Sparkle update feed for Cycler. Regenerated by Scripts/sparkle-appcast.sh per release. -->
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" xmlns:dc="http://purl.org/dc/elements/1.1/">
  <channel>
    <title>Cycler</title>
    <link>https://cycler.caiano.com/appcast.xml</link>
    <description>Updates for Cycler, a macOS window manager.</description>
    <language>en</language>
    <item>
      <title>Cycler ${VERSION}</title>
      <link>${NOTES}</link>
...
echo "==> wrote web/appcast.xml for Cycler ${VERSION} (build ${BUILD})"
echo "    enclosure: ${URL}"
echo "    Next: confirm the DMG is uploaded to that GitHub release, then commit web/appcast.xml"
echo "    (it auto-deploys to https://cycler.caiano.com/appcast.xml)."
